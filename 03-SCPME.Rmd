...
where $A \in \mathbb{R}^{m \times p}, B \in \mathbb{R}^{p \times q}, \mbox{ and } C \in \mathbb{R}^{m \times q}$ are matrices that are assumed to be known and specified so that solving the full penalized gaussian log-likelihood for $\Omega$ results in solving
...
where $L$, still the *augmented lagrangian*, is defined as
...
Instead of solving the first step exactly as we did in the previous section, the authors propose an alternative, approximating objective function, which we will call $\tilde{L}$, that is based on the majorize-minimize principle^[Further explanation of the majorizing function \@ref(eq:approx) in section \@ref(taylorsexp)]. The purpose of this approximating function is the desire to solve this first step in closed-form. The ADMM algorithm with this modification based on majorize-minimize principle is also found in @lange2016mm but here the approximating function is defined as
...
where $Q = \tau I_{p} - \left(A^{T}A \otimes BB^{T}\right)$ and $\tau$ is chosen such that $Q$ is positive definite and thus $L_{\rho}\left(\cdot\right) \leq \tilde{L}\left(\cdot\right)$ for all $\Omega$ (therefore $\tilde{L}$ is a majorizing function^[Note that if $Q$ is positive definite, then $vec\left(\Omega - \Omega^{k} \right)^{T}\rho Q\left(\Omega - \Omega^{k} \right)/2 > 0$ since $\rho > 0$ and $vec\left(\Omega - \Omega^{k}\right)$ is always nonzero whenever $\Omega \neq \Omega^{k}$.]).

The *augmented ADMM* algorithm developed by Molstad and Rothman, which now includes the majorize-minimize principle, consists of the following repeated iterations:
...
A possibe stopping criterion for this framework is one derived from similar optimality conditions used in the previous section \@ref(ADMMstop). The primal optimality condition here is that $A\Omega^{k + 1}B - Z^{k + 1} - C = 0$ and the two dual optimality conditions are $0 \in \partial f\left(\Omega^{k + 1}\right) + \left(B(\Lambda^{k + 1})^{T}A + A^{T}\Lambda^{k + 1}B^{T} \right)/2$ and $0 \in \partial g\left(Z^{k + 1}\right) - \Lambda^{k + 1}$. Similarly, we will define the left-hand side of the primal optimality condition as the primal residual $r^{k + 1} = A\Omega^{k + 1}B - Z^{k + 1} - C$ and the dual residual^[Proof of \@ref(eq:stopproof) in section \@ref(proofstopproof).] as
...
Of course, in practice we do not know the true covariance matrix $\Sigma_{xy}$ but we might consider using the sample estimate $\hat{\Sigma}_{xy} = \sum_{i = 1}^{n}\left(X_{i} - \bar{X}\right)\left(Y_{i} - \bar{Y}\right)^{T}/n$ and then using our estimator, $\hat{\Omega}_{x}$, to construct the forward regression coefficient matrix estimator $\hat{\beta} = \hat{\Omega}_{x}\hat{\Sigma}_{xy}$. Like @witten2009covariance, this approach estimates the forward regression coefficient matrix while using shrinkage estimators of the marginal precision matrix for the predictors. Specifically, this optimization problem has the effect of deriving an estimate of $\Omega_{x} \mbox { and } \beta$ while assuming sparsity in the forward regression coefficient $\beta$.

The penality outline in the SCPME framekwork is flexible that many novel estimators can be constructed. For instance, we might also consider taking $B = \left[ \Sigma_{xy}, I_{p} \right]$ so that the identity matrix is appended to the sample cross-covariance matrix of $X$ and $Y$. In this case, not only are we assuming that $\beta$ is sparse, but we are also assuming sparsity in $\Omega$.
...
In this simulation, we compare the performance of several competing prediction methods, including the so-called SCPME regression estimators discussed in the previous section, under various data realizations. In total, we consider the following estimators:

 - `OLS` = ordinary least squares estimator. In *high* dimensional setting ($p >> n$), the Moore-Penrose estimator is used.
 ...
 - `oracleO` = regression estimator with oracle $\Omega^{*}$ (let $\beta = \Omega^{*}\hat{\Sigma}_{xy}$).
 
 - `oracleS` = regression estimator with oracle $\Sigma_{xy}^{*}$ (let $\beta = \hat{\Omega}\Sigma_{xy}^{*}$).
 
 - `shrinkB` = SCPME regression estimator with penalty $\lambda\left\| \Omega\hat{\Sigma}_{xy} \right\|_{1}$.
 
 - `shrinkBS` = SCPME regression estimator with oracle $\Sigma_{xy}^{*}$ so that the penalty is $\lambda\left\| \Omega\hat{\Sigma}_{xy}^{*} \right\|_{1}$.
 
 - `shrinkBO` = SCPME regression estimator with penalty $\lambda\left\| \Omega[\Sigma_{xy}, I_{p}] \right\|_{1}$.
 
 - `glasso` = graphical lasso estimator.
 
Each tuning parameter was selected to minimize the mean squared prediction error (MSPE) over 3-fold cross validation.

For each data generation procedures, let $\mathbb{X} \in \mathbb{R}^{n \times p}$, $\mathbb{Y} \in \mathbb{R}^{n \times r}$, and $\beta \in \mathbb{R}^{p \times r}$ so that $\mathbb{Y} = \mathbb{X}\beta + \mathbb{E}$. Here we let the script notation denote the fact that the columns have been centered so as to remove the intercept in our prediction models. The regression coefficient matrix $\beta$ is constructed so that $\beta = \mathbb{B} \circ \mathbb{V}$ where $vec\left( \mathbb{B} \right) \sim N_{pr}\left( 0, I_{p} \otimes I_{r}/\sqrt{p} \right)$ and $\mathbb{V} \in \mathbb{R}^{p \times r}$ is a matrix containing $p$ times $r$ random bernoulli draws with 50\% probability being equal to one. The residual covariance matrix $\Sigma_{y | x}$ is constructed so that $\left( \Sigma_{xx} \right)_{ij} = 0.7^{\left| i - j \right|}$ and $\left( \Sigma_{y | x} \right)_{ij} = 0.7^{\left| i - j \right|}$, respectively. This ensures that its corresponding precision matrix will be tridiagonal and sparse. Then for 100 independent, identically distributed samples, we have that $X_{i} \sim N_{p}\left( 0, \Sigma_{xx} \right)$ and $E_{i} \sim N_{r}\left( 0, \Sigma_{y | x} \right)$. An additional 1000 samples will be generated for the testing set and the methods were evaluated on both model error and mean squared prediction error. Each data generating procedure for each $p$ in $(10, 50, 100, 150)$ was replicated a total of 20 times.
