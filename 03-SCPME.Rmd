...
The table below shows the average model error for each estimator when $n = 100$, $p = 150$, and $r = 10$.
...
This final simulation was run and explored due to the fact each of the SCPME regression estimators require an estimation of the cross-covariance matrix. In the previous simulations, we used the maximum likelihood estimator. However, because the data-generating procedures construct settings that are inherently sparse, we were interested to determine if additional shrinkage of the *cross-covariance* matrix would also prove beneficial. For instance, in a number of settings, we were finding that the estimator `oracleO` ($\beta = \Omega_{x}^{*}\hat{\Sigma}_{xy}$) was performing worse than `oracleS` ($\beta = \hat{\Omega}_{x}\Sigma_{xy}^{*}$) which perhaps suggests that estimating the covariance matrix $\Sigma_{xy}$ well is *more* important than estimating $\Omega$ well. This simulation explores that theory a bit further.

The data-generating procedure for this simulation is similar to previous one but here we take $n = 100$, $p = 200$, $r = 10$, *and* we additionally multiple the cross-covariance sample estimator by a factor of $k$ where $k \in (0.1, 0.2, ..., 0.9, 1)$. Figure \@ref(fig:scpmesim7) plots the MSPE for `shrinkB` for each tuning parameter, $\lambda$, and constant, $k$, pair. Figure \@ref(fig:scpmesim9) plots the MSPE similarly for `shrinkBO`.
